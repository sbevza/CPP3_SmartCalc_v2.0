...
MainWindow::MainWindow(s21::Controller *c, QWidget *parent)
    ...
    QDoubleValidator *dbValidator = new QDoubleValidator(-100000.0, 100000.0, 2, this);
...
//          ui->percent->setValidator(doubleValidator);
    ...
    //            int pos = 0;
    //            if (ui->x_line->validator()->validate(inputText, pos) != QValidator::Acceptable) {
    //                ui->x_line->setStyleSheet("background-color: red;");
    //            } else {
    //                ui->x_line->setStyleSheet("");
    //            }
    //        });
    connect(ui->x_line, SIGNAL(textChanged(const QString&)), this, SLOT(on_x_line_textChanged(const QString&)));
    ...
    connect(ui->pushButton_minus, SIGNAL(clicked()), this, SLOT(operate_button()));
    ...
    connect(ui->pushButton_bracket_l, SIGNAL(clicked()), this, SLOT(operate_button()));
    connect(ui->pushButton_bracket_r, SIGNAL(clicked()), this, SLOT(operate_button()));
}
...
void MainWindow::on_x_line_textChanged(const QString &arg1) {
    ...
    if (ui->x_line->validator()->validate(inputText, pos) != QValidator::Acceptable) {
        ...
        ui->x_line->setStyleSheet("border: 1px solid white;\n	color: white;\n	font-family: Arial;\n	"
                                  "font-size: 14pt;\n	font-weight: 600;\n	margin-bottom: 4px;");
    }
}
...
void MainWindow::on_pushButton_del_x_clicked() {
    ui->x_line->setText("");
}
